...
from dgt_coin.processor.handler import make_dgt_address


LOGGER = logging.getLogger(__name__)

def loads_dgt_token(data,name):
    value = cbor.loads(base64.b64decode(data))[name]
    token = DgtTokenInfo()
    token.ParseFromString(value)
    LOGGER.debug("DGT:%s %s=%s",name,token.group_code,token.decimals)
...
def create_dgt_transaction(verb, name, value, signer,to = None):
    payload = DgtPayload(verb=verb, name=name, value=value, to = to)

    # The prefix should eventually be looked up from the
    # validator's namespace registry.
    addr = make_dgt_address(name)
    inputs  = [addr]
    outputs = [addr]
    if to is not None:
        addr_to = make_dgt_address(to)
