...
from dgt_coin.client_cli.generate import add_generate_parser
from dgt_coin.client_cli.generate import do_generate
from dgt_coin.client_cli.populate import add_populate_parser
from dgt_coin.client_cli.populate import do_populate
from dgt_coin.client_cli.create_batch import add_create_batch_parser
from dgt_coin.client_cli.create_batch import do_create_batch
from dgt_coin.client_cli.load import add_load_parser
from dgt_coin.client_cli.load import do_load
from dgt_coin.client_cli.dgt_workload import add_workload_parser
from dgt_coin.client_cli.dgt_workload import do_workload

from dgt_coin.client_cli.dgt_client import DgtClient
from dgt_coin.client_cli.exceptions import DgtCliException
from dgt_coin.client_cli.exceptions import DgtClientException
from dgt_common.protobuf.dgt_token_pb2 import DgtTokenInfo

DISTRIBUTION_NAME = 'dgt-coin'
...
def add_set_parser(subparsers, parent_parser):
    message = 'Sends an DGT transaction to set <name> to <value>.'

    parser = subparsers.add_parser(
        'set',
        parents=[parent_parser],
        description=message,
        help='Sets an DGT value')
...
def add_inc_parser(subparsers, parent_parser):
    message = 'Sends an DGT transaction to increment <name> by <value>.'

    parser = subparsers.add_parser(
        'inc',
        parents=[parent_parser],
        description=message,
        help='Increments an DGT value')
...
def add_dec_parser(subparsers, parent_parser):
    message = 'Sends an DGT transaction to decrement <name> by <value>.'

    parser = subparsers.add_parser(
        'dec',
        parents=[parent_parser],
        description=message,
        help='Decrements an DGT value')
...
def add_trans_parser(subparsers, parent_parser):
    message = 'Sends an DGT transaction from <name> to  <to> by <value>.'

    parser = subparsers.add_parser(
        'trans',
        parents=[parent_parser],
        description=message,
        help='transfer an DGT value from vallet to vallet')
...
def add_show_parser(subparsers, parent_parser):
    ...
    parser = subparsers.add_parser(
        ...
        parents=[parent_parser],
        description=message,
        help='Displays the specified DGT value')
...
def add_list_parser(subparsers, parent_parser):
    message = 'Shows the values of all keys in DGT state.'

    parser = subparsers.add_parser(
        'list',
        parents=[parent_parser],
        description=message,
        help='Displays all DGT values')
...
def main(prog_name=os.path.basename(sys.argv[0]), args=None):
    ...
    if args.command == 'set':
        ...
        raise DgtCliException("invalid command: {}".format(args.command))
...
def main_wrapper():
    ...
    try:
        main()
    except (DgtCliException, DgtClientException) as err:
