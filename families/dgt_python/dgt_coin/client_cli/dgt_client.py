...
from dgt_coin.processor.handler import make_dgt_address,FAMILY_NAME,FAMILY_VER
    ...
    def __init__(self, url, keyfile=None):
        ...
        if keyfile is not None:
            try:
                ...
                raise DgtClientException(
                    'Failed to read private key: {}'.format(str(err)))

            try:
                private_key = Secp256k1PrivateKey.from_hex(private_key_str)
            except ParseError as e:
                raise DgtClientException('Unable to load private key: {}'.format(str(e)))
    ...
    def _send_request(self, suffix, data=None, content_type=None, name=None):
        ...
        try:
            ...
            if result.status_code == 404:
                raise DgtClientException("No such key: {}".format(name))

            elif not result.ok:
                raise DgtClientException("Error {}: {}".format(
