...
from dgt_coin.client_cli.workload.workload_generator import WorkloadGenerator
from dgt_coin.client_cli.workload.dgt_workload import Workload
from dgt_coin.client_cli.create_batch import create_dgt_transaction
from dgt_coin.client_cli.create_batch import create_batch
from dgt_coin.client_cli.exceptions import DgtCliException
from dgt_sdk.protobuf import batch_pb2

LOGGER = logging.getLogger(__name__)

DgtState = namedtuple('DgtState', ['name', 'url', 'value'])
    ...
    def __init__(self, delegate, args):
        ...
        if args.key_file is not None:
            try:
                ...
                raise DgtCliException(str(pe))
            except IOError as ioe:
                raise DgtCliException(str(ioe))
        else:
            self._signer = crypto_factory.new_signer(
            ...
            if code == 202:
                with self._lock:
                    self._pending_batches[batch_id] = \
                        DgtState(name=name, url=url, value=0)
...
def do_workload(args):
    ...
    try:
        ...
        workload = DgtWorkload(generator, args)
