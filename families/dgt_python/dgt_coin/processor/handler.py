...
FAMILY_NAME = 'dgt'
FAMILY_VER  = '1.0'
DGT_ADDRESS_PREFIX = hashlib.sha512(FAMILY_NAME.encode('utf-8')).hexdigest()[0:6]


def make_dgt_address(name):
    return DGT_ADDRESS_PREFIX + hashlib.sha512(name.encode('utf-8')).hexdigest()[-64:]


class DgtTransactionHandler(TransactionHandler):
    def __init__(self):
        ...
        LOGGER.info('DgtTransactionHandler init DONE')
    ...
    def apply(self, transaction, context):
        ...
        LOGGER.debug('apply: _do_dgt.\n')
        updated_state = self._do_dgt(verb, name, value, to, state)

        _set_state_data( updated_state, context)

    def _do_dgt(self,verb, name, value, to, state):
        verbs = {
            'set': self._do_set,
            'inc': self._do_inc,
            'dec': self._do_dec,
    ...
    def _do_set(self,name, value, to, state):
        ...
        token = DgtTokenInfo(group_code = 'DGT_token',
                             owner_key = self._signer.sign('DGT_token'.encode()), #owner_key,
...
def _get_state_data(name,to, context):
    states = [make_dgt_address(name)]
    if to is not None:
        states.append(make_dgt_address(to))
